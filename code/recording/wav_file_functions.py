...
def detect_audio_device():
    ...
    position =np.nan # Initialize position variable
    for idx, device in enumerate(devices):  # Iterate through devices with index
        if 'USB Audio' in device['name']:  # Check if 'USB Audio' is in the device name
            position = idx  # Save index as position
            break  # Optional: break loop if only the first occurrence is needed
